...
function Headers(value) {
    return function (target, propertyName) {
        const config = util_1.getConfig(target, propertyName);
        if (0 && propertyName == null) {
            config.headers = config.headers || {};
            config.headers.common = merge_1.default(config.headers.common || {}, value);
        }
        else {
            config.headers = merge_1.default(config.headers || {}, value);
        }
        util_1.setConfig(config, target, propertyName);
    };
}
