...
function getCookieJar(target, propertyName) {
    let config = reflect_metadata_util_1.getMetadataLazy(util_1.SymConfig, target, propertyName);
    if (!config || !config.jar) {
        config = reflect_metadata_util_1.getMetadataLazy(util_1.SymConfig, target);
    }
    if (!config || !config.jar) {
