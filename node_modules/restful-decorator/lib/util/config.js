...
function _getSetting(thisArgv, requestConfig, propertyName) {
    const baseURL = reflect_metadata_util_1.getMetadataLazy("BASE_URL" /* BASE_URL */, thisArgv, propertyName);
    if (propertyName == null) {
        merge_1.merge(requestConfig, reflect_metadata_util_1.getMetadataLazy(util_1.SymConfig, thisArgv));
    }
    if (baseURL != null) {
        // @ts-ignore
        requestConfig.baseURL = url_1.default(baseURL);
    }
    const config = util_1.getConfig(thisArgv, propertyName);
    //console.log(555);
    //console.dir(config);
    merge_1.merge(requestConfig, config);
...
function _chkSettingUpdate(defaults, opts) {
    ...
    if (ks.length) {
        ...
        if (bool) {
            //console.dir(opts);
            defaults = merge_1.mergeClone(defaults, opts);
...
function fixRequestConfig(requestConfig) {
    ...
    if (requestConfig.method === 'GET') {
        if (requestConfig.data) {
            requestConfig.params = merge_1.merge(requestConfig.params, requestConfig.data);
